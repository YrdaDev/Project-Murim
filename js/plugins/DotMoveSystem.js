...
/*:
@target MV MZ
@plugindesc Dot movement system v2.2.5 - Optimized a bit by Sang Hendrix
@author unagi ootoro
...
(function (DotMoveSystem) {
    ...
    class MapCharactersCache {
        ...
        addMapCharactersCache(mass, character) {
            ...
            if (this._cache[mass].size > 20) {
                const currentCharacters = Array.from(this._cache[mass]);
                const playerX = $gamePlayer.x;
                const playerY = $gamePlayer.y;

                currentCharacters.sort((a, b) => {
                    const distA = Math.abs(a.x - playerX) + Math.abs(a.y - playerY);
                    const distB = Math.abs(b.x - playerX) + Math.abs(b.y - playerY);
                    return distA - distB;
                });

                this._cache[mass] = new Set(currentCharacters.slice(0, 15));
            }
        }
    ...
    class CharacterCollisionChecker {
        ...
        checkCollisionCharacters(x, y, d, targetCharacterClass) {
            ...
            // Pre-filter characters by rough distance before expensive collision checks
            const roughCheckDistance = Math.max(this._character.width(), this._character.height()) + 2;

            for (const character of this.enteringMassesCharacters(x, y)) {
                if (!(character instanceof targetCharacterClass)) continue;

                const dx = Math.abs(character._realX - x);
                const dy = Math.abs(character._realY - y);
                if (dx > roughCheckDistance || dy > roughCheckDistance) continue;
    ...
    class CharacterDotMoveProcess {
        ...
        calcDistance(deg) {
            const cacheKey = `${deg.value}_${this._dpf}`;
            if (!this._distanceCache) this._distanceCache = new Map();

            let cached = this._distanceCache.get(cacheKey);
            if (!cached) {
                cached = DotMoveUtils.calcDistance(deg, this._dpf);
                ...
                this._distanceCache.set(cacheKey, cached);
            }
    ...
    Game_Character.prototype.findDirectionTo = function (goalX, goalY, searchLimit = Math.min(this.searchLimit(), 10)) {
